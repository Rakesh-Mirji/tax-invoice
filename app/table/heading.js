      ...
      <form onSubmit={onCreateBtnClick}>
        <div className="flex gap-10 p-5 flex-wrap justify-center">
          {Object.keys(heading).map(elem => 
            <input key={elem} className="max-md:w-[50vw] p-1 rounded-lg h-10 w-[20vw] border-2"  name={elem} placeholder={`Heading number ${parseInt(elem)+1}`} ></input>
            )}
        </div>
        <div className="flex gap-10 w-full  pt-10 justify-around">
          {Object.keys(heading).length > 0 && <button className="py-3 px-5 border-2 rounded-lg bg-blue-500 text-white duration-300" type="submit" >Create Table</button>}
        </div>
      </form>
      </div>
    </>)
